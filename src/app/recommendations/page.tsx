...
import { RECOMMENDATIONS, RECOMMENDATION_CATEGORIES, type RecommendationCategory } from '@/data/recommendations'
    ...
    <main className="relative min-h-screen" style={{ background: '#060a13' }}>
      ...
      <div className="relative py-24 px-6 max-w-4xl mx-auto" style={{ zIndex: 1 }}>
        ...
        <div className="flex flex-wrap justify-center gap-2 mb-12">
          {RECOMMENDATION_CATEGORIES.map(({ value, label }) => {
            const isActive = activeCategory === value
            const color = value === 'all' ? '#00d4aa' : (CATEGORY_COLORS[value] ?? '#00d4aa')
            return (
              <button
                key={value}
                onClick={() => setActiveCategory(value as RecommendationCategory | 'all')}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((rec, i) => {
            ...
            return (
              ...
              >
                {rec.image && (
                  <div className="w-full h-32 rounded-lg mb-4 overflow-hidden bg-white/5">
