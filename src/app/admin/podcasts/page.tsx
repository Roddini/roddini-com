  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => { setForm({ ...emptyForm, sort_order: podcasts.length }); setEditingId(null); setShowForm(true) }} className="px-4 py-2 rounded bg-white/10 hover:bg-white/20 text-sm transition-colors">
          + Add
        </button>
      </div>
      ...
      {showForm && editingId === null && (
        <div className="mb-6 rounded-lg bg-white/5 p-4 flex flex-col gap-3">
          ...
          <label className="flex items-center gap-2 text-sm text-white/50">Position
              ...
              <span className="text-white/30">of {editingId ? podcasts.length : podcasts.length + 1}</span>
            </label>
