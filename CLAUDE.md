...
- `/admin/resume` — upload a résumé PDF (or paste text) → Claude parses it into structured experience entries → review/edit each (role, company, period, year, description, highlights, tags, accent, order) → **Publish** (replace-all into `experience`). The uploaded PDF also becomes the downloadable résumé. See Résumé import below.
